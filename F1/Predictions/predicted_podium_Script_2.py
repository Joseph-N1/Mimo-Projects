...
def predict_and_output(drivers_df, target_race_name, races_used):
    ...
    'track_start_avg': 0.00     # Remove track starting position
        ...
        # In predict_and_output function:
        ...
        for feature in manual_weights.keys():
           ...
           if max_val - min_val == 0:
            ...
            # INVERSE normalization for features where lower is better
               drivers_df[feature + "_norm"] = (max_val - drivers_df[feature]) / (max_val - min_val)
              else:
            # Standard normalization for features where higher is better
               drivers_df[feature + "_norm"] = (drivers_df[feature] - min_val) / (max_val - min_val)
               
        # Apply weights to normalized features
        drivers_df['podium_score'] = sum([
            drivers_df[feature + "_norm"] * weight for feature, weight in manual_weights.items()
        ])
