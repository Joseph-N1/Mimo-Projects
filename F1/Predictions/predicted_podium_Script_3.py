...
def train_models(X, y_class):
    ...
    try:
        ...
        manual_weights = {
    'recent_form': 0.28,        # Slight increase for current performance
    'avg_starting_position': 0.25,  # Most important factor
    'dnf_rate': 0.15,           # Increased DNF impact
    'avg_pos_change': 0.12,     # Position change importance
    'avg_quali_improvement': 0.08,
    'hot_streak': 0.05,
    'fastest_lap_count': 0.04,
    'track_podiums': 0.02,      # Reduced track history
    'track_avg_pos': 0.01,      # Minimal track history
    'track_start_avg': 0.00     # Remove track starting position
        }
...
def predict_and_output(drivers_df, target_race_name, races_used):
    try:
        ...
        # Optimized weights based on Monaco analysis
        manual_weights = {
            'avg_starting_position': 0.38,
            'recent_form': 0.25,
            'track_podiums': 0.15,
            'track_avg_pos': 0.08,
            'avg_quali_improvement': 0.04,
            'hot_streak': 0.04,
            'dnf_rate': 0.02,
            'avg_pos_change': 0.02,
            'fastest_lap_count': 0.01,
            'track_start_avg': 0.01
        }

        print("🔧 Using optimized manual scoring")

        # Define inverse normalization features
        inverse_features = {'avg_starting_position', 'dnf_rate', 'track_avg_pos', 'track_start_avg'}

        # Apply intelligent normalization
        for feature in manual_weights.keys():
            min_val = drivers_df[feature].min()
            max_val = drivers_df[feature].max()
            range_val = max_val - min_val
            
            if range_val == 0:
                drivers_df[feature + "_norm"] = 0
            else:
                if feature in inverse_features:
                    # Inverse: lower values are better (e.g., starting position)
                    drivers_df[feature + "_norm"] = (max_val - drivers_df[feature]) / range_val
                else:
                    # Standard: higher values are better (e.g., recent form)
                    drivers_df[feature + "_norm"] = (drivers_df[feature] - min_val) / range_val
        
        # Calculate weighted podium score
        drivers_df['podium_score'] = sum(
            drivers_df[feature + "_norm"] * weight 
            for feature, weight in manual_weights.items()
        )
